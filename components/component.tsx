...
export function Component() {
  ...
  const registerForAuction = async () => {
    ...
    if (!account) {
      setMessage(
        "Error: no account to register for auction. Go generate an account first."
      );
      return;
    }
  ...
  const placeBid = async () => {
    ...
    if (!account) {
      window.alert(
        "Error: no account to place bid. Go generate an account first."
      );
      return;
    }
    ...
    if (!price || price <= 0) {
      window.alert(
        "Error: no valid price to place bid. Enter a valid price and try again."
      );
      return;
    }
    ...
    if (!file) {
      window.alert(
        "Error: no proof file uploaded. Go generate a proof and upload it first."
      );
      return;
    }
...
/*
  ...
  */
  ...
  return (
    <div>
      <img
        src={`${process.env.PUBLIC_URL}/zkbid_blue.png`}
        alt="Description"
      ...
      >
        <TabsList className="grid grid-cols-4 gap-4 flex flex-wrap">
          <TabsTrigger
            value="account"
            disabled={DEBUG ? false : progress < Progress.ACCOUNT}
          >
            Generate Account
          </TabsTrigger>
          <TabsTrigger
            value="register"
            disabled={DEBUG ? false : progress < Progress.REGISTER}
          >
            Register for Auction
          </TabsTrigger>
          <TabsTrigger
            value="proof"
            disabled={DEBUG ? false : progress < Progress.PROOF}
          >
            Generate Proof
          </TabsTrigger>
          <TabsTrigger
            value="bid"
            disabled={DEBUG ? false : progress < Progress.BID}
          >
            Place Bid
          </TabsTrigger>
        </TabsList>
        <TabsContent value="account">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Creates a keypair for a temporary account and stores it on the
                frontend.
              </CardDescription>
            </CardHeader>
        ...
        <TabsContent value="register">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Sends an HTTPS request to the backend to fetch a voucher.
              </CardDescription>
            </CardHeader>
            <CardContent className="grid gap-4">
              {account && (
        ...
        <TabsContent value="proof">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Takes private key of main account(one that is funded) and
                generates proof that it have enough funds to place a bid.
                Instructions for generating proof locally:
              </CardDescription>
            </CardHeader>
            <CardContent className="grid gap-4">
              <CopyBlock
        ...
        <TabsContent value="bid">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Sends a message to the auction contract using the fetched
                voucher and generated account.
              </CardDescription>
            </CardHeader>
            <CardContent className="grid gap-4">
              {account && (
              ...
              >
                ...
                >
                  <h5>
                    Drag and drop your proof file here, or click to select file
                    from disk.
                  </h5>
                  <br />
                  <h5>
